...
def scrape_mit(n_articles = 10):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for url in links_mit[:n_articles]:
...
def scrape_iea(n_articles = 10):
    ...
    results = []
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for url in links_iea[:n_articles]:
            future = executor.submit(fetch, url)
            results.append(future)
    results = [future.result() for future in results]
...
def scrape_rn(n_articles = 10):
    ...
    results = []
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for url in links_rn[:n_articles]:
            future = executor.submit(fetch, url)
            results.append(future)
    results = [future.result() for future in results]
...
def scrape_mi(n_articles = 10):
    ...
    results = []
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for url in links_mi[:n_articles]:
            future = executor.submit(fetch, url)
            results.append(future)
    results = [future.result() for future in results]
...
def scrape(n_articles_per_source = 10):
    ...
    data['id'] = [uuid.uuid1() for _ in range(len(data))]
